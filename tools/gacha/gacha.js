...
(function () {
  ...
  /** n 連するときに、実際に石で払う回数。
   *
   * **チケットは「70 回募集時」にもらえる。**つまり 70 連目までは石で払い、
   * 無料になるのは 71〜80 連目。`n >= 70` で 10 連ぶん引くと、**ちょうど 70 連の
   * ときに 60 連ぶんしか払っていないことになる**（2026-09-24 に見つけた）。
   * 公式のお知らせ 679「生徒募集システムのリニューアルについて」の
   * 「70回募集時：期間限定10回募集チケット×1」がその形。
   */
  function paidPulls(n) {
    if (n <= 0) return 0;
    var t = n;
    for (var i = 0; i < BONUS_AT.length; i++) {
      if (n > BONUS_AT[i]) t -= Math.min(10, n - BONUS_AT[i]);
    }
    return t;
  }
  ...
  /**
   * 募集の種類ごとの提供割合。**★3 の内訳がまるごと違う。**
   * 周年・限定では★3 の合計が 2 倍になるが、**ピックアップの 0.7% は動かない。**
   * 増えるのはすり抜け（周年の他の生徒と、通常の★3）の側。
   */
  var BANNERS = {
    normal: {
      label: 'ピックアップ募集',
      ...
      // **100 カウント目の確定★3 は、いつもの割合では割れない。**
      // そこだけ「50% がピックアップ」で、残りをすり抜けの側で比例配分する。
      // 通常募集はすり抜けが 1 本しかないので、そのまま 50%
      at100: [0.5, 0.5],
      star2: 0.185, star1: 0.785
    },
    fes: {
      label: '周年・限定（確率アップ）',
      star3: 0.060,
      ...
      // 残り 50% を、すり抜け 2 本の比（0.009 : 0.044）で分ける
      at100: [0.5, 0.5 * 0.009 / 0.053, 0.5 * 0.044 / 0.053],
      star2: 0.185, star1: 0.755
    }
  };
