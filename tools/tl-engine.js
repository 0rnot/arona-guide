...
(function () {
  ...
  /* **戦闘が始まってすぐは貯まらない。**回復が動き出すのは 2.0 秒後。

     2026-08-30 は kur-3dcg の TL 作成支援ツールの `RECOVERY_DELAY_MS = 2033`
     に合わせて 2.033 にしていた。**2026-09-01 に 2.0 へ戻した。**
     ぱちみ（@pach_8353）の「総力戦 TL Planner」に入っている
     ビナー TORMENT の実例 TL 21 発と突き合わせたところ、
     **8 発が揃って 0.0330 秒（1 フレーム）ぶんこちらが遅く**、
     差はすべてこの定数ぶんだった。
       ミネ    向こう  11.5238 秒 ／ こちら  11.5568 秒
       マキ    向こう  54.3810 秒 ／ こちら  54.4140 秒
       マキ    向こう  85.3333 秒 ／ こちら  85.3663 秒 …（8 発とも +0.0330）
     コスト 4 ÷ 0.42/秒 ＝ 9.5238 秒。向こうの 11.5238 秒は遅れ 2.000 秒ちょうど。
     **ゲーム側の `Excel/ConstCombatExcelTable.json` の
     `PlayerRegenCostDelay` も 2000**（＝2.0 秒）で、出どころ 2 つが一致する。
     2033 は動画 1 本の実測なので、そちらを採らない。 */
  var REC_DELAY = 2.0;
