...
(function () {
  ...
  /* ---------- 計算は ../tl-engine.js に置いてある

     **このファイルが DOM を読む唯一の層。**エンジンは純関数で、画面の状態は
     `env()` が集めた `input` オブジェクトで渡す（形はあちらの冒頭のコメント）。
     TL エディタ（`tools/tl/`）も同じエンジンを呼ぶので、
     **計算を足すならこちらではなく `tools/tl-engine.js` に足す。**

     下の別名は、**呼び出し側を書き換えないため**に置いてある。
     `forms(d)` などは今までどおりの綴りのまま使える。 */
  var E = window.TLENGINE;
  var LAYOUT = E.LAYOUT, MAIN_MAX = E.MAIN_MAX, SUP_MAX = E.SUP_MAX;
  var W4_CAP = E.W4_CAP, START_COST = E.START_COST, OVER_FLOOR = E.OVER_FLOOR;
  var REC_DELAY = E.REC_DELAY, TICK_MAX = E.TICK_MAX, FORM_RULE = E.FORM_RULE;
  var forms = E.forms, autoForm = E.autoForm, ovlMs = E.ovlMs, extend = E.extend;
  var atLv = E.atLv, bfRow = E.bfRow, bfTiered = E.bfTiered, isPct = E.isPct;
  var timedOf = E.timedOf, condOf = E.condOf, mkBar = E.mkBar, redraws = E.redraws;
  // **`-0` を出さない**丸め。`simulate` の中でも使うのでエンジン側が正本
  var n1 = E.n1;

  /** 編成で決まるぶんだけの `input`。**入力欄は読まない。**
      `members()` のように 1 回の描画で何十回も呼ばれるものがここを通る。 */
  function env() {
    return { D: D, mode: mode, slots: slots, order: order, gims: gims };
  }
  function isMain(i) { return E.isMain(i); }
  function live(i) { return E.live(mode, i); }
  function members() { return E.members(env()); }
  function effects() { return E.effects(env()); }
  ...
  function pool() {
    var i = env();
    i.base = D.base;
    i.gb = parseFloat(el('i-gb').value) || 0;
    i.gc = parseFloat(el('i-gc').value) || 0;
    return E.pool(i);
  }
  function simulate(cap, start, base, gb, gc) {
    var i = env();
    i.cap = cap; i.start = start; i.base = base; i.gb = gb; i.gc = gc;
    return E.simulate(i);
  }
  function collectBars(sim, span) {
    var i = env();
    i.show = show; i.span = span;
    return E.collectBars(i, sim);
  }
  ...
  /* ---------- EX 以外のスキル（ノーマル・パッシブ・サブ）を画面に出す

     読み分け（`timedOf` / `condOf`）と帯そのもの（`mkBar`）は `../tl-engine.js`。
     ここに残しているのは、**その結果を日本語にして出すぶんだけ。**

     **`iv`（発動間隔・秒）はゲームのデータには欄が無い。**スキル文の「N秒毎に」を
     読んだ値で、読めたのは `ns` を持つ 175 人のうち 142 人。読めなかった 33 人は
     `iv: 0` のまま、引き金の原文が `cond` に入っている——そちらは時刻が
     決められないので帯にせず、「条件で発動するもの」の一覧に回す。 */
