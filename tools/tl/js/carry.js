...
export function ggAbsorbRuns(r, g) {
  ...
  try {
    ...
    // **通常攻撃をゲージに数えるのは取り下げた**（2026-09-04、動画で確かめて確定）。
    // 「いちばん近い敵に当たる」の根拠にした `TargetSortRule` は**スキルの相手選び**で、
    // 通常攻撃の相手を決める表は `DB/` 456 表のどこにも無い
    // （`CharacterAIExcelTable` 70 行は寄り方だけ。`Target` / `Aggro` / `Priority` を
    //  持つ表も当たった）。**動画のコマで数えて決めた**——`fr_y4h8XEXXfgw/` の
    // ボス HP バーの下に**グロッキーゲージが 3 マス**で出ていて、
    //   コマ 1〜42   0 マス
    //   コマ 45〜95  1 マス目が満杯・2 マス目が半分（≒ 47%）
    //   コマ 99〜116 3 マス満杯 → **ボスの目が渦・頭に星＝グロッキー**
    // **240 秒で 1 回だけ。**通常攻撃を数えると 4 回立って、平均が実測 40,020,560 に
    // 対して 60,335,789（+51%）まで膨らむ。数えない今の形は 42,513,830（+6%）で、
    // ゲージの折れ線も [18,0] [36,0] [56,5004] [74,5004] [94,10000] と動画に合う
...
/** `t0` からボスの時計で `need` 秒ぶん進んだ実時刻。**いまは素通し。**

    「転倒しているボスは通常攻撃を撃たないから時計も止まる」を一度入れて外した
    （2026-09-04）。**木に `ApplyGroggy` の行が 1 つも無く、DB は止まるとも進むとも
    言っていない。**動画でも決められない——`y4h8XEXXfgw` はグロッキーが
    240 秒で 1 回（コマ 99〜116）しか立たないので、止まるかどうかが出ない。
    入れても抜いても平均は 1% も動かなかった。**根拠が無いものは入れない。**
    形だけ残してあるのは、根拠が出たときにここだけ直せばよいから。 */
