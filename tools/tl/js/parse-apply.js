...
export function applyTL1(p) {
  ...
  for (i = 0; i < p.uses.length; i++) {
    ...
    if (u.rem != null && u.md === 't') {
      // **前の行にいちばん近い分を採る**（2026-09-04）。コスト指定・最速の行の
      // 解けた時刻は engine の見積もりで前後にずれるので、「前の行より後ろ」を
      // 固い条件にすると、正しい分を数秒差で弾いて**丸ごと 1 分後ろへ飛ぶ**。
      // 大決戦ペロロジラ `WPsUxtkDMQU` の「07.80　ホシノ」がこれで、前の行 59.1 秒に
      // 対して 52.2 秒（残り 3:07.80）を 6.9 秒の差で弾き、112.2 秒（残り 2:07.80）に
      // 置いて**53 秒の穴**を空けていた。前は「5 秒だけ手前まで許す」だったが、
      // 秒数を足すのは当てずっぽうなので、**差がいちばん小さい分**にした。
      // 同じ差なら後ろ側（書いてある順を守る側）を採る
      var rk = null, mk, bestD = Infinity;
      for (mk = 0; mk * 60 + u.rem <= dur; mk++) {
        var cnd = mk * 60 + u.rem, at9 = dur - cnd;
        var d9 = Math.abs(at9 - prev) - (at9 >= prev ? 1e-6 : 0);
        if (d9 < bestD) { bestD = d9; rk = cnd; }
      }
