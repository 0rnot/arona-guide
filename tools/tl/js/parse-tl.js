...
export function parseTL(txt) {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    b.pot = [25, 25, 25];
    // **3 つ並べる書き方**（「WB0-25-0」「WB25/25/0」）。**並びは HP / 攻撃 / 治癒**で、
    // 下の 1 つだけ書く形と同じ順。クラウさんの概要欄で確かめた（2026-09-08）:
    // 制服ネル（アタッカー）が `WB0-25-0`、水着セイア（回復役）が `WB25-0-25`、
    // リオが `WB25-25-0`、水着イブキが `WB0-25-25`、ホシノ（臨戦。壁）が `WB25/25/0`。
    // **真ん中が攻撃**でないとアタッカーの攻撃が 0 になる。
    // ここが無くて `WB0-25-0` が `[0,0,0]`・`WB25/25/0` が `[25,25,25]` に潰れていた
    // （ビナー IrVUx0ywuyo で ネルの攻撃が 9,281。書き手の 8,591＋支援 1,163 ＝ 9,754 より 473 低い）
    mm = rest.match(/(?:WB|潜在|限界突破)\s*(\d+)\s*[-\u2010\u2212\u30fc/／]\s*(\d+)\s*[-\u2010\u2212\u30fc/／]\s*(\d+)/i);
    if (mm) {
      b.pot = [Math.min(+mm[1], 25), Math.min(+mm[2], 25), Math.min(+mm[3], 25)];
      mm = null;
    } else {
  ...
  for (i = 0; i < lines.length; i++) {
    ...
    // **「名前 + 区切り + 合図」は合図ではなく使用の行**（2026-09-03）。
    // `cbAthbwldys` の「ナツ ビーム出たら」が合図として落ちて 1 発足りず、
    // 同じ TL の「ハレ 歩いたら」は（`歩いたら` が下の並びに無いので）落ちていない、
    // という不揃いだった。**分かれ目は名前のあとに区切りがあるか。**
    // 大決戦ケセドの「ハナコの銃が床についたら」は名前のあとが「の」で続く 1 文で
    // 区切りが無いので、今までどおり合図として落ちる。
    // **「〜NS」で終わる行には効かせない**（「ハナコ NS」は撃つ子ではなく引き金）
    // **`nrm` は空白を落とすので、ここは生の行で見る**
    var nmCue = String(noParen).match(/^[\s　]*([^\s　、,・]+)[\s　]+(.+)$/);
    var cueIsUse = !!(nmCue && whoIn(nmCue[1], crew) >= 0);
    ...
    if (aim == null && mcn === 1 && idBy[who]) {
      ...
      if (hasAr) {
        ...
        // **既定は「巻き込める最大数」**（2026-09-04 の先生の指示
        // 「巻き込んで攻撃する数は調整であえて少なくしたりするから、
        //   デフォルトを巻き込める最大数、任意で数を選べるって感じにできる？」）。
        // `bestHitsOf()` が狙い方（中心の置き方・向き）を動かして、いちばん多く
        // 覆える置き方を探す。少なくしたいときは行の「当たる数」で下げる。
        // **`spn`（1 回の湧きの数）は「盤に何体居るか」であって
        // 「1 発が何体に当たるか」ではない。**ペロロジラの大きなペロロ 6 体は
        // (0,31) (±3.5,30) (±3.5,32) (2,30) に散っていて最小包含円が半径 3.64 ワールド、
    ...
    // **形態 0 が持続効果を持っているなら、それが本命の札**（2026-09-03、61d）。
    // 「ダメージが無い」だけで送ると、イブキの
    // 「自身を除く円形範囲内の味方の ATK を増加（35 秒間）」が一度も乗らず、
