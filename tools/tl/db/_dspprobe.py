"""解除（`DispelLogicEffectTemplate` ほか）が何を何枚剥がしたかを数える。

    ./venv/bin/python tools/tl/db/_dspprobe.py <vid> [--seed 0]

出すのは `R.dsp`（`<撃った枠の GroupId>><剥がされた体>:<剥がす札の名前>` → 枚数）。
0 枚のものも出す（空振りが分かる）。
"""
import argparse
import json
...
async ([key, dur, seed]) => {
  ...
  return {killAt: r.killAt, maxHp: r.maxHp, total: r.total,
          hpEnd: r.hp.length ? r.hp[r.hp.length - 1] : null,
          deaths: r.deaths || null, dsp: r.dsp || null};
}
"""
...
def main():
    ...
    print('total:', m['total'])
    print('deaths:', json.dumps(m['deaths'], ensure_ascii=False)[:600])
    dsp = m['dsp'] or {}
    print(f"dsp {len(dsp)} 通り")
    for k, v in sorted(dsp.items(), key=lambda kv: -kv[1]):
        print(f"  {v:6d}  {k}")
    return 0
